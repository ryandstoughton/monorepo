...
export const createWorktree = async (options) => {
    const branch = options.branchStrategy.type === "branch"
        ...
        : undefined;
    const baseBranch = options.branchStrategy.type === "branch"
    ...
    const worktreeInteractive = async (opts) => {
        ...
        try {
            return await Effect.runPromise(inner.pipe(Effect.provide(ClackDisplay.layer), Effect.provide(NodeContext.layer), Effect.provide(NodeFileSystem.layer)));
        }
        catch (error) {
    ...
    const worktreeRun = async (opts) => {
        ...
        if (opts.resumeSession) {
            ...
            if (!existsSync(sessionPath)) {
                throw new Error(`resumeSession "${opts.resumeSession}" not found: expected session file at ${sessionPath}`);
            }
        }
        const inner = Effect.gen(function* () {
            // 1. Resolve prompt
